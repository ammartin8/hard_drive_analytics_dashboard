...
CREATE OR REPLACE EXTERNAL TABLE `my-project-name.hard_drive_dataset.hard_drive_data_tbl_ext`
OPTIONS (
  format = 'PARQUET',
  uris = ['gs://my-bucket-name/raw_data/*.parquet']
);
...
CREATE OR REPLACE TABLE `my-project-name.hard_drive_dataset.hard_drive_data_tbl`
...
FROM my-project-name.hard_drive_dataset.hard_drive_data_tbl_ext
;
